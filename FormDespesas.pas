...
procedure TFrmDespesas.BitBtn2Click(Sender: TObject);
var
...
begin
   ...
   with FDQDespesas do begin
      ...
      ParamByName('despfixa').AsInteger := 1;
...
procedure TFrmDespesas.AtualizarGridBotao;
begin
   with FDQueryGrid do begin
      Close;
      SQL.Clear;
      //Adicione parcelas e vlrparcelas
      SQL.Text := 'SELECT chaveclifor, valor, descricao  FROM movfinanceirascf where despfixa = 1';
      try
         Open;
...
procedure TFrmDespesas.AtualizarGrid(SQL: string);
begin
...
procedure TFrmDespesas.EdtFimKeyPress(Sender: TObject; var Key: Char);
var
   Posicao: Integer;
   TempText: string;
begin
...
procedure TFrmDespesas.EdtInicioExit(Sender: TObject);
begin
...
procedure TFrmDespesas.EdtInicioKeyPress(Sender: TObject; var Key: Char);
var
   Posicao: Integer;
   TempText: string;
begin
...
procedure TFrmDespesas.EdtParcelasExit(Sender: TObject);
begin
   if EdtParcelas.Text = '1' then
      EdtVlrParcelas.Text := EdtValor.Text
   else
      EdtVlrParcelas.Text := IntToStr((StrToInt(EdtValor.Text)) * (StrToInt(EdtParcelas.Text)));
end;
